...
from lass_audio.jukebox.hparams import Hyperparams, setup_hparams, REMOTE_PREFIX
from lass_audio.jukebox.utils.remote_utils import download
from lass_audio.jukebox.utils.torch_utils import freeze_model
from lass_audio.jukebox.utils.dist_utils import print_all
from lass_audio.jukebox.vqvae.vqvae import calculate_strides
import fire
...
def make_prior(hps, vqvae, device='cuda'):
    from lass_audio.jukebox.prior.prior import SimplePrior
    ...
    if hps.fp16_params:
        ...
        from lass_audio.jukebox.transformer.ops import _convert_conv_weights_to_fp16
...
def save_outputs(model, device, hps):
    ...
    for level in range(len(priors)):
        ...
        y_in = t.from_numpy(prior.labeller.get_y_from_ids(
            artist_id, genre_ids, lyric_tokens, total_length, offset)).view(1, -1).long()
